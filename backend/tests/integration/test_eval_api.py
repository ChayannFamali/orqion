...
async def _login_as_developer(
    api_client: httpx.AsyncClient,
    app_fixture: FastAPI,
) -> None:
    """Login as developer (no manage_corpora capability) for access control tests."""
    from app.policy.presets import BUILTIN_ROLES

    factory = app_fixture.state.db_session_factory
    async with factory() as session:
        ws = Workspace(name="test-dev")
        session.add(ws)
        await session.flush()
...
# ---------------------------------------------------------------------------
# Access control (Gate 3 item 3)
# ---------------------------------------------------------------------------


async def test_eval_list_denied_for_non_manage_corpora(
    api_client: httpx.AsyncClient,
    app_fixture: FastAPI,
) -> None:
    """Developer without manage_corpora gets 404 on eval endpoints."""
    await _login_as_developer(api_client, app_fixture)
