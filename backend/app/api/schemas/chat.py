...
class ChatRequest(BaseModel):
    ...
    # T-445 (каркас): выбор режима рассуждения на уровне сообщения (вариант Г1).
    # Значения "on"/"off"; учитывается только при политике роли
    # "optional" (off/on фиксируют режим и выбор игнорируется). Конкретный
    # параметр запроса провайдеру пока не отправляется — параметр добавится,
    # когда найдётся живой провайдер с наблюдаемым переключением.
