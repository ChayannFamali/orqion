...
async def agent_chat(
    ...
    # ``approve`` исполняет подтверждённый вызов НАПРЯМУЮ (модель не обязана
    # запрашивать его повторно) — разрешение действует на конкретный вызов
    # (имя и аргументы должны совпасть с показанными пользователю), после
    # чего модель строит финальный ответ по результату.
