...
class SettingValueInvalid(OrqionError):
    """Значение ключа настройки не прошло валидацию pydantic-моделью спеки.

    Статус 422: это не «нет права» (404) и не «нет такого ключа» (404), а
    некорректное значение. Отдельный класс вместо ``RequestValidationError``
    — чтобы отказ пришёл в единой плоской форме ``{error, reason,
    constraint, hint}``, как остальные ошибки API, и с текстом на русском:
    сообщение pydantic показывается пользователю в поле формы. Конкретное
    требование (тип, диапазон, список опций) собирается из описания поля и
    уходит в ``hint``.
    """
